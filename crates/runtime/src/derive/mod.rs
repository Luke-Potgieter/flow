    ...
    H: Fn(&ops::Log) + Send + Sync + Clone + 'static,
{
        ...
        In: futures::Stream<Item = tonic::Result<Request>> + Send + Unpin + 'static,
    {
        ...
        let response_rx = match endpoint {
            ...
            models::DeriveUsing::Connector(models::ConnectorConfig { image, .. }) => {
                ...
                let response_rx = image::image_connector(
                    image,
                    self.log_handler,
    ...
    R: Stream<Item = tonic::Result<Request>> + Send + 'static,
{
    request_rx.inspect_ok(move |request| {
        let Some(Ok(DeriveRequestExt{labels: Some(ops::ShardLabeling { log_level, .. }), ..})) = request.get_internal() else { return };
