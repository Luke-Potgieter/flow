...
impl Child {
    ...
    /// Check whether the Child has exited without blocking, returning Ok(None) if the child is still running.
...
pub async fn input_output(cmd: &mut Command, input: &[u8]) -> std::io::Result<Output> {
    ...
    let (_, _, _, wait) = tokio::join!(
        // Wrapping future is required to drop `stdin_pipe` once `input` is written or fails.
